...
import com.kei.moviecatalogue.model.movie.UpcomingResponse
...
class UpcomingMovieAdapter (val listUpcoming : List<UpcomingResponse>) : RecyclerView.Adapter<UpcomingMovieAdapter.UpcomingMovieViewHolder>(){
    class UpcomingMovieViewHolder(val view : View) : RecyclerView.ViewHolder(view) {
        fun bind(upcomingMovies : UpcomingResponse){
            with(itemView){
                Glide.with(context).load(BuildConfig.IMAGE_URL + upcomingMovies.poster_path).into(iv_upcoming)
                tv_title_upcoming.text = upcomingMovies.title
                tv_description_upcoming.text = upcomingMovies.overview
    ...
    override fun onBindViewHolder(holder: UpcomingMovieAdapter.UpcomingMovieViewHolder, position: Int) {
